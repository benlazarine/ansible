"""
This 'CLI' will allow you to instantiate a PythonPlaybookRunner from the REPL:
    ```
    from ansible.cli.python import PythonPlaybookRunner
    runner = PythonPlaybookRunner(
        ["/opt/dev/atmosphere-ansible/ansible/playbooks/utils/hello_world.yml"],
        extra_vars={'test_key':'test-value'},
        subset=['ip-or-target-or-group-name-defined-in-hosts'],
        inventory="/opt/dev/atmosphere-ansible/ansible/hosts",
        config_file="/opt/dev/atmosphere-ansible/ansible/ansible.cfg"
    )
    runner.run()
    ```
"""
...
class PythonPlaybookRunner(PlaybookCLI):
    ...
    def __init__(self, args, callback=None, extra_vars={}, **parser_kwargs):
        ...
        """
        super(PythonPlaybookRunner, self).__init__(args, callback=callback)
        ...
        if 'logger' not in parser_kwargs:
            parser_kwargs['logger'] = default_logger
    ...
    def parse(self, args, parser_kwargs):
        ...
        super(PythonPlaybookRunner, self).parse(args, values)
    ...
    def _select_register_vars(self, temporary_fact_cache):
        ...
        sanitized_vars = {}
        for host, facts in temporary_facts_dict.items():
            ...
            if sanitized_fact_keys:
                sanitized_facts = dict((k, v) for (k, v) in facts.items() if k in sanitized_fact_keys)
